...
export function extractAndConvertPattern(text: string): PatternMatch[] {
  ...
  const patterns = [
    ...
    /["'“”]*([あ-んア-ンA-ZＡ-Ｚa-zａ-ｚ])["'“”]*(?:ブロック)?\s*[-ｰ_ー－‐＿]?\s*([0-9０-９]{1,2})\s*([abａｂ]+)/gi,
  ];
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const ab = match[3]; // ab
      converted = toHalfWidth(`${block}${number}${ab}`);
...
// 日程情報を抽出・正規化（複数日程に対応）
export function extractDay(text: string): string {
  ...
  // 土曜日・土曜を1日目に変換
  if (/土曜日/i.test(normalized) || /土曜/i.test(normalized)) {
    days.push("1日目");
  }

  // 日曜日・日曜を2日目に変換
  if (/日曜日/i.test(normalized) || /日曜/i.test(normalized)) {
    days.push("2日目");
  }
  ...
  if (/(?:^|[^曜])土(?:[^曜]|$)/i.test(normalized)) {
    days.push("1日目");
  }

  if (/(?:^|[^曜])日(?:[^曜目]|$)/i.test(normalized)) {
    days.push("2日目");
  }
